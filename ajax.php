<?php
	...
	if($type != ""){
		switch($type){
			case "search":
				...
				$limit = isset($_POST["limit"]) ? htmlspecialchars(strip_tags(trim($_POST["limit"]))) : "";
				echo searchGame($name,$limit);
				break;
			case "getGameDetails":
	...
	function searchGame($name, $limit){
		require_once 'src/Unirest.php';
		
		$url = "https://igdbcom-internet-game-database-v1.p.mashape.com/games/?fields=name&limit=" . $limit . "&offset=0&order=release_dates.date%3Adesc&search=" . $name;
